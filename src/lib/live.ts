...
export function getLiveDefaultUA() {
  return defaultUA;
}

export async function getLiveSourceByKey(sourceKey?: string | null) {
  if (!sourceKey) return null;
  const config = await getConfig();
  return config.LiveConfig?.find((live) => live.key === sourceKey) || null;
}
