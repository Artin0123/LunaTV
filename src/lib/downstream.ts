...
function parseVodPlayUrl(vodPlayUrl?: string): {
  ...
  const vodPlayUrlArray = vodPlayUrl.split('$$$');
...
export async function getDetailFromApi(
  ...
  const parsedPlay = parseVodPlayUrl(videoDetail.vod_play_url);
  let episodes = parsedPlay.episodes;
  const titles = parsedPlay.titles;
