...
- [x] **5.40** 替换 `next-pwa`（已停止维护）为 `@serwist/next` 或 `@ducanh2912/next-pwa`
...
- [x] **6.1** 拆分 `db.client.ts`（1643 行） → 多个模块（已完成：`src/lib/client/`）
...
- [x] **6.23** 拆分 `src/app/admin/page.tsx`（5476 行）（已完成：`src/app/admin/_components/`）
- [x] **6.24** 拆分 `src/lib/db.client.ts`（1643 行）（已完成：`src/lib/client/`）
...
| 阶段四：安全改进        |   4    |    ✅ 完成    |
| 阶段五：依赖升级        |   40   | 🔄 31/40 完成 |
| 阶段六：可选优化        |   28   | 🔄 21/28 完成 |
| 阶段七：实时监控        |   5    |    ✅ 完成    |
| **合计**                | **94** |   **78/94**   |
